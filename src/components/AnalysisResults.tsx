            ...
            <div className="space-y-3">
              ...
              <div className="bg-white dark:bg-gray-800 rounded-lg p-3 border border-gray-200 dark:border-gray-700">
                <div className="flex flex-wrap items-center gap-1.5">
                    <button
                      onClick={() => {
                        setSelectedSeverities(new Set(['error']));
                        setSelectedTypes(new Set(['security']));
                      }}
                      className="h-9 px-3 text-xs font-medium bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300 rounded-md hover:bg-red-200 dark:hover:bg-red-900/50 transition-colors"
                    >
                      Critical
                    </button>
                    <button
                      onClick={() => {
                        setSelectedSeverities(new Set(['error', 'warning']));
                        setSelectedTypes(new Set());
                      }}
                      className="h-9 px-3 text-xs font-medium bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-300 rounded-md hover:bg-orange-200 dark:hover:bg-orange-900/50 transition-colors"
                    >
                      High Priority
                    </button>
                    <button
                      onClick={() => {
                        setSelectedTypes(new Set(['performance']));
                        setSelectedSeverities(new Set());
                      }}
                      className="h-9 px-3 text-xs font-medium bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300 rounded-md hover:bg-yellow-200 dark:hover:bg-yellow-900/50 transition-colors"
                    >
                      Performance
                    </button>
                    <button
                      ...
                      className="h-9 px-3 text-xs font-medium bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-md hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors"
                    >
                      Clear
                    </button>
                    {(selectedTypes.size > 0 || selectedSeverities.size > 0 || searchQuery) && (
                      <span className="ml-auto h-9 px-3 inline-flex items-center text-xs text-gray-600 dark:text-gray-400 bg-gray-100 dark:bg-gray-700 rounded-md">
                        {filteredResults.length}/{activeReports.reduce((acc, r) => acc + r.results.length, 0)}
                      </span>
                    )}
                </div>
              </div>
